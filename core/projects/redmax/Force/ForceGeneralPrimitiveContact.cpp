...
ForceGeneralPrimitiveContact::ForceGeneralPrimitiveContact(
    ...
    if (_primitive_body == nullptr) {
        throw_error("The second body in Contact should be primitive body.");
    }
...
void ForceGeneralPrimitiveContact::set_friction(dtype mu) {
    _mu = mu;
}

void ForceGeneralPrimitiveContact::set_damping(dtype damping) {
    _damping = damping;
}

void ForceGeneralPrimitiveContact::set_scale(dtype scale) {
    _scale = scale;
}

void ForceGeneralPrimitiveContact::computeForce(VectorX& fm, VectorX& fr, bool verbose) {
    auto t0 = clock();
...
void ForceGeneralPrimitiveContact::computeForceWithDerivative(
    ...
    Km.block(_contact_body->_index[0], _primitive_body->_index[0], 6, 6) += Km_sub.topRightCorner(6, 6);
    Km.block(_primitive_body->_index[0], _contact_body->_index[0], 6, 6) += Km_sub.bottomLeftCorner(6, 6);
    Km.block(_primitive_body->_index[0], _primitive_body->_index[0], 6, 6) += Km_sub.bottomRightCorner(6, 6);
    
    Dm.block(_contact_body->_index[0], _contact_body->_index[0], 6, 6) += Dm_sub.topLeftCorner(6, 6);
    Dm.block(_contact_body->_index[0], _primitive_body->_index[0], 6, 6) += Dm_sub.topRightCorner(6, 6);
    Dm.block(_primitive_body->_index[0], _contact_body->_index[0], 6, 6) += Dm_sub.bottomLeftCorner(6, 6);
    Dm.block(_primitive_body->_index[0], _primitive_body->_index[0], 6, 6) += Dm_sub.bottomRightCorner(6, 6);
...
void ForceGeneralPrimitiveContact::computeForce(std::vector<Contact> &contacts, VectorX& fm, bool verbose) {
    ...
    for (int i = 0;i < contacts.size();i++) {
        ...
        dtype d, ddot;
        Vector3 n, tdot, xi2;
        _primitive_body->collision(xw1, xw1_dot, d, n, ddot, tdot, xi2);

        if (verbose) {
            std::cerr << "General Primitive Contact: d = " << d << ", contact body = " << _contact_body->_name << ", primitive body = " << _primitive_body->_name << ", xi1 = " << xi1.transpose() << std::endl;
            ...
            std::cerr << "centor = " << _primitive_body->_E_0i.topRightCorner(3, 1).transpose() << std::endl;
        }
        ...
        if (verbose && dynamic_cast<BodyCuboid*>(const_cast<Body*>(_contact_body)) != nullptr) {
            std::cerr << "contact_body = " << _contact_body->_name << ", primitive body = " << _primitive_body->_name << ", xi1 = " << xi1.transpose() << ", fc1 = " << fc1.transpose() << std::endl;
        }
...
void ForceGeneralPrimitiveContact::computeForceWithDerivative(
    ...
    for (int i = 0;i < contacts.size();i++) {
        ...
        // contact force on primitive contact body
        Vector6 fc2 = (kn * d - _damping * ddot * d) * ni2;
        ...
        if (_mu > constants::eps) {
            ...
            // frictional force on the primitive contact body
            if (_mu * fc_norm >= kt * tdot_norm - constants::eps) {  // static frictional force
                Vector6 fs = kt * GTRT2 * tdot;
                fm.tail(6) += fs;
...
void ForceGeneralPrimitiveContact::computeForceWithDerivative(
    ...
    for (int i = 0;i < contacts.size();i++) {
        ...
        for (auto ancestor = _contact_body->_joint; ancestor != nullptr; ancestor = ancestor->_parent) {
            if (ancestor->_design_params_1._active) {
                ...
                for (int k = 0;k < ndof;k++) {
                    int idx = idx_start + k;
                    dfm_dp.block(_primitive_body->_index[0], idx, 6, 1) += 
        ...
        if (_contact_body->_design_params_6._active) {
            int idx = _contact_body->_design_params_6._param_index(0) + p6_offset;
            dfm_dp.block(_primitive_body->_index[0], idx, 6, 1) += kn * d * ni2 / _contact_body->_contact_scale;
        }
        ...
        if (_mu > constants::eps) {
            ...
            if (_mu * fc_norm >= kt * tdot_norm - constants::eps) {  // static frictional force
                ...
                if (_contact_body->_design_params_6._active) {
                    int idx = _contact_body->_design_params_6._param_index(0) + p6_offset;
                    dfm_dp.block(_primitive_body->_index[0], idx, 6, 1) += fs2 / _contact_body->_contact_scale;
                }
            } else {                                // dynamic frictional force
                RowVector6 dfc_norm_dq1 = (1. / fc_norm) * (fc1.transpose() * dfc1_dq1);
                RowVector6 dfc_norm_dq2 = (1. / fc_norm) * (fc1.transpose() * dfc1_dq2);
                ...
                if (_contact_body->_design_params_6._active) {
                    int idx = _contact_body->_design_params_6._param_index(0) + p6_offset;
                    dfm_dp.block(_primitive_body->_index[0], idx, 6, 1) += fd2 / _contact_body->_contact_scale;
                }
            }
        }
    }
...
void ForceGeneralPrimitiveContact::test_derivatives() {
    std::cerr << "**************************** General-Primitive Collision Derivatives ***************************" << std::endl;
    // srand(time(0));
    srand(1000);
    ...
    this->_primitive_body->_E_0i = E2;
    this->_primitive_body->_phi = phi2;
    ...
    for (int i = 0;i < 6;i++) {
        ...
        this->_primitive_body->_E_0i = E2_pos;
        VectorX fm_pos;
        computeForce(contacts, fm_pos);
    ...
    for (int i = 0;i < 6;i++) {
        ...
        phi2_pos[i] += eps;
        this->_primitive_body->_phi = phi2_pos;
        VectorX fm_pos;
        computeForce(contacts, fm_pos);
...
void ForceGeneralPrimitiveContact::test_derivatives_runtime() {
    // detect the contact points between two bodies
    std::vector<Contact> contacts;
    contacts.clear();
    collision_detection_general_primitive(_contact_body, _primitive_body, contacts);
    
    // test derivative for primitive body
    for (int i = 0;i < contacts.size();i++) {
        Vector3 xi1 = contacts[i]._xi;
        Vector3 xw1 = _contact_body->_E_0i.topLeftCorner(3, 3) * xi1 + _contact_body->_E_0i.topRightCorner(3, 1);
        ...
        _primitive_body->test_collision_derivatives_runtime(xw1, xw1_dot);
    }
    ...
    MatrixX E2 = this->_primitive_body->_E_0i;
    VectorX phi2 = this->_primitive_body->_phi;
    ...
    print_error("General-Primitive Collision Derivatives: Km(q1) ", Km.leftCols(6), Km_fd.leftCols(6));
    ...
    this->_primitive_body->_E_0i = E2;
    print_error("General-Primitive Collision Derivatives: Km(q2) ", Km.rightCols(6), Km_fd.rightCols(6));
    ...
    this->_contact_body->_phi = phi1;
    print_error("General-Primitive Collision Derivatives: Dm(phi1) ", Dm.leftCols(6), Dm_fd.leftCols(6));
    ...
    this->_primitive_body->_phi = phi2;
    print_error("General-Primitive Collision Derivatives: Dm(phi2) ", Dm.rightCols(6), Dm_fd.rightCols(6));

    test_design_derivatives_runtime();
}

void ForceGeneralPrimitiveContact::test_design_derivatives_runtime() {
    // detect the contact points between two bodies
    std::vector<Contact> contacts;
    contacts.clear();
    ...
    if (contacts.size() > 0) {
        ...
        for (int ii = 0;ii < 1;ii++) {
            ...
            for (int i = 0;i < _sim->_ndof_p;i++) {
                ...
                fm_pos.segment(_primitive_body->_index[0], 6) += fm_sub_pos.tail(6);
                dfm_dp_fd.col(i) = (fm_pos - fm) / eps;
            }
            // std::cerr << "eps = " << eps << std::endl;
            print_error("General-Primitive Collision Derivatives: dfm_dp1", dfm_dp.leftCols(_sim->_ndof_p1), dfm_dp_fd.leftCols(_sim->_ndof_p1));
            print_error("General-Primitive Collision Derivatives: dfm_dp2", dfm_dp.middleCols(_sim->_ndof_p1, _sim->_ndof_p2), dfm_dp_fd.middleCols(_sim->_ndof_p1, _sim->_ndof_p2));
            print_error("General-Primitive Collision Derivatives: dfm_dp3", dfm_dp.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2, _sim->_ndof_p3), dfm_dp_fd.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2, _sim->_ndof_p3));
            print_error("General-Primitive Collision Derivatives: dfm_dp6", dfm_dp.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2 + _sim->_ndof_p3 + _sim->_ndof_p4 + _sim->_ndof_p5, _sim->_ndof_p6), dfm_dp_fd.middleCols(_sim->_ndof_p1 + _sim->_ndof_p2 + _sim->_ndof_p3 + _sim->_ndof_p4 + _sim->_ndof_p5, _sim->_ndof_p6));
            eps /= 10.;
        }
